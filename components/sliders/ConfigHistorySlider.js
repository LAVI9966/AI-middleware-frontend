...
function ConfigHistorySlider({ versionId }) {
  ...
  const labels = useMemo(() => Object.fromEntries(CONFIG_HISTORY_FEATURE_OPTIONS.map((o) => [o.value, o.label])), []);
  ...
  const { draftItems, historyItems } = useMemo(
    () => splitDraftAndHistory(visible, lastPublishedAt),
    [visible, lastPublishedAt]
  );
  ...
  const renderHistoryRow = (item, id, isDraft = false) => (
    <HistoryRow
      ...
      onToggle={() => toggle(id)}
      showRevert={item?.type !== "Version published" && !isSystemHistoryType(item?.type)}
    ...
    >
      <div className="flex flex-col gap-4 h-full min-h-0">
        ...
        <div className="bg-base-100 rounded-lg p-4 border border-base-300 shrink-0 space-y-3">
          ...
          <button onClick={reset} className="w-full py-1.5 text-xs bg-base-300 rounded hover:bg-base-200">
            Clear
          </button>
        </div>
